...
**어?중간**은 “어디서 만나?”라는 질문을 링크 하나로 해결하는 모임 장소 추천 서비스입니다. 여러 참석자의 출발 위치를 모은 뒤 **기하학적 중앙값(geometric median)** 을 참고점으로 계산하고, 그 좌표를 그대로 목적지로 쓰지 않고 실제 음식점·카페가 모인 상권을 다시 탐색해 **추천 지역 1·2·3**을 보여줍니다.

> **어디서 만나? 중간에서. — 어?중간**

GitHub 저장소와 Pages 주소는 기존 호환성을 위해 계속 `meet-halfway`를 사용합니다. 서비스 브랜드만 **어?중간 / EO?JUNGAN**으로 표시됩니다.

![어?중간 Social Preview](./repository-social-preview.png)

## Preview

생성자는 모임 이름, 자신의 이름, 출발 위치, 입력 마감을 정해 링크를 만듭니다. 참석자는 같은 링크에서 이름과 출발 위치를 추가합니다. 마감 후에는 수학적 중간점을 그대로 목적지로 쓰지 않고 주변 상권을 탐색해 추천 1·2·3 지역을 제시합니다. 추천 지역을 선택하면 지도가 해당 상권으로 확대되고 음식점·카페 핀이 지도 위에 나타납니다. 핀을 누르면 상세 오버레이가 열리며, 아래 리스트 카드를 눌러서도 해당 핀으로 이동할 수 있습니다.
...
- **보조 영문 표기:** EO?JUNGAN
- **한 줄 문구:** 어디서 만나? 중간에서.
- **Repository / Pages path:** `meet-halfway` 유지
- **Logo concept:** 물음표는 “어디서 만나?”를, 양옆 출발점과 중앙의 점은 “중간에서 만남”을 뜻합니다.

브랜드 변경은 화면/메타데이터/README/OG/PWA 아이콘에만 적용하며, 기존 공유 링크와 LocalStorage 호환성을 위해 `meet-halfway` 내부 식별자와 URL 경로는 바꾸지 않습니다.
...
- **중간점 우선 추천권** 안에서 음식점·카페 밀도와 지하철/철도 거점 데이터를 함께 비교해 실제 상권 후보를 최대 3곳 추천
- 추천 카드·지도 번호 핀 모두 클릭 가능하며, 같은 추천지를 다시 누르면 선택 해제 후 전체 추천권으로 복귀
- **Three.js 3D 추천 포인트 뷰**: 수학적 중간점을 코어로 두고 추천 1·2·3의 실제 상대 위치를 입체적으로 표시하며, 3D 포인트/라벨 클릭으로 선택·해제 가능
- WebGL 미지원·CDN 차단·저사양 환경에서는 동일 기능의 2D 체크포인트 UI로 자동 폴백
- 추천 지역 선택 시 지도 자동 확대 및 음식점·카페 핀 최대 60개 표시
- 지도 핀 클릭 시 상호명·카테고리·주소·거리·전화/평점(공급자가 제공하는 경우)·주소 복사·공유·외부지도 링크 오버레이
- 아래 장소 카드를 누르면 해당 지도 핀으로 즉시 이동
- 음식점 / 카페 / 간편식·주점 카테고리 필터와 장소 리스트
- 추천 2·3을 채우기 위해 도시 전체로 반경을 넓히지 않으며, 중심 추천권에 후보가 부족할 때만 제한적으로 확장 후보를 사용
- `#/m/demo`에서는 외부 API 장애와 관계없이 추천 3곳 + 지도 핀 UI를 확인할 수 있는 명시적 예시 데이터 제공
- 한국어 / English / 日本語 / 中文
...
- Three.js (runtime-loaded WebGL recommendation visualization, graceful 2D fallback)
...
## 3D Recommendation Visualization

결과 화면의 3D 추천 포인트는 지도 자체를 대체하지 않습니다. 실제 위치·식당 탐색은 Kakao/Leaflet/Google 지도에서 유지하고, Three.js는 **중간점과 추천 상권의 상대 위치·선택 상태를 빠르게 이해하기 위한 시각 레이어**로 사용합니다.

- 중앙 코어: 수학적 중간점
- 3개의 입체 체크포인트: 추천 1·2·3
- 선택된 포인트: 글로우/링/높이 강조
- 포인트 또는 라벨 클릭: 후보 선택, 다시 클릭: 선택 해제
- `prefers-reduced-motion` 사용자는 정적 렌더링
- WebGL 또는 Three.js CDN 사용이 불가능하면 2D 후보 선택 UI 유지

Three.js는 빌드 dependency로 묶지 않고 브라우저에서 버전 고정 CDN 모듈로 지연 로드합니다. 따라서 첫 화면 로딩과 GitHub Pages의 무의존성 빌드 구조는 그대로 유지됩니다.
...
```text
Name: meet-halfway
Description: Eo?JungGan - where should we meet? Three practical midpoint areas with nearby food and cafes
...
Initial tag: v1.5.0
Initial commit/update: feat: refresh Eo?JungGan branding
```
...
If the Kakao Maps Web SDK cannot load because the Kakao app is missing a JavaScript SDK domain, the map product is disabled, or the wrong key type is used, 어?중간 automatically falls back to OpenStreetMap/Nominatim/Overpass so meeting creation and Supabase verification can continue.
...
The GitHub bootstrap is idempotent for an existing `v1.5.0` tag. A pre-existing remote release tag is kept instead of causing deployment to fail after a successful Pages deployment.
