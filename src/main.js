...
function externalMapUrl(point) {
  ...
  if (isKakaoMapsConfigured) return `https://map.kakao.com/link/map/${encodeURIComponent(point.name || point.areaName || 'MeetHalfway')},${point.lat},${point.lng}`;
...
function candidateNearbyPlaces(candidate, allPlaces) {
  if (!candidate) return [];
  return placesNear(candidate, allPlaces, 2.2, 10);
}
...
function resultsHtml(snapshot, allPlaces, hotspots, selectedIndex = 0, selectedCategory = 'all') {
  ...
  const selected = hotspots[selectedIndex] || hotspots[0] || null;

  const candidateCards = hotspots.length ? hotspots.map((hotspot, index) => {
    const isSelected = selected?.id === hotspot.id;
    const areaName = nearestPlaceName(hotspot, allPlaces);
    return `
      <article class="candidate-card ${isSelected ? 'is-selected' : ''}">
        <div class="candidate-card-head"><span class="candidate-rank">${escapeHtml(t('results.candidate', { n: index + 1 }))}</span>${index === 0 ? '<span class="candidate-best">BEST</span>' : ''}</div>
        ...
        <div class="candidate-actions"><button type="button" class="button ${isSelected ? 'button--primary' : 'button--ghost'} button--small" data-candidate-index="${index}">${escapeHtml(t('results.selectArea'))}</button><a href="${externalMapUrl(hotspot)}" target="_blank" rel="noreferrer">${escapeHtml(t('results.openMap'))}</a></div>
      </article>`;
  ...
  if (selected) {
    ...
    const placeCards = visible.length ? visible.map((place) => `
      <article class="place-card">
        <div class="place-card-main"><span class="place-category place-category--${escapeHtml(place.category || 'restaurant')}">${escapeHtml(categoryLabel(place.category))}</span><h4>${escapeHtml(place.name)}</h4><p>${escapeHtml(place.address || place.cuisine || t('results.dataSource'))}</p></div>
        <div class="place-card-side"><strong>${place.distanceKm.toFixed(1)}km</strong><a href="${externalMapUrl(place)}" target="_blank" rel="noreferrer">${escapeHtml(t('results.openPlace'))} ↗</a></div>
    ...
    detail = `
      <section class="selected-area-card">
        <div class="selected-area-heading"><div><span class="eyebrow">${escapeHtml(t('results.selectedArea'))}</span><h3>${escapeHtml(nearestPlaceName(selected, allPlaces))}</h3><p>${escapeHtml(t('results.selectedHint'))}</p></div><a class="button button--ghost button--small" href="${externalMapUrl(selected)}" target="_blank" rel="noreferrer">${escapeHtml(t('results.openMap'))}</a></div>
  ...
  return `
    <div class="result-summary"><div><span>${escapeHtml(t('results.avgDistance'))}</span><strong>${avg.toFixed(1)}km</strong></div><div><span>${escapeHtml(t('results.maxDistance'))}</span><strong>${max.toFixed(1)}km</strong></div><div><span>${escapeHtml(t('results.placeCount'))}</span><strong>${allPlaces.length}</strong></div><div><span>${escapeHtml(t('results.recommendationCount'))}</span><strong>${hotspots.length}</strong></div></div>
    <div class="recommendation-explainer"><strong>${escapeHtml(t('results.title'))}</strong><span>${escapeHtml(t('results.recommendationIntro'))}</span></div>
...
function activateResultExperience(resultPanel, snapshot, allPlaces, hotspots, center) {
  ...
  const paint = () => {
    ...
    resultPanel.querySelectorAll('[data-candidate-index]').forEach((button) => {
      button.addEventListener('click', () => {
        selectedIndex = Number(button.dataset.candidateIndex) || 0;
...
async function renderMeeting(shareId) {
  ...
  try {
    ...
    if (closed) {
      ...
      try {
        const places = await discoverMeetingPlaces(calculatedCenter, snapshot.participants);
        if (version !== routeVersion) return;
