...
import { DEMO_SNAPSHOT } from './lib/demo.js';
...
import { findRestaurants } from './lib/places.js';
...
function applyDocumentLanguage() {
  ...
  document.title = `${t('brand.en')} — ${t('home.title2')}`;
  ...
  return `
    <header class="site-header">
      <a class="brand" href="#/" aria-label="${escapeHtml(t('brand'))}">
        <span class="brand-mark" aria-hidden="true"><span></span><span></span><i></i></span>
        <span class="brand-copy"><strong>${escapeHtml(t('brand'))}</strong><small>MeetHalfway</small></span>
      </a>
...
function footerHtml() {
  ...
  return `
    <footer class="site-footer">
      <div><strong>${escapeHtml(t('brand'))}</strong><span class="footer-product">MeetHalfway</span></div>
  ...
  app.innerHTML = `
    ...
    <main id="main">
      <section class="hero shell">
        ...
        <div class="hero-visual" aria-label="MeetHalfway preview">${heroMapHtml()}</div>
      </section>
...
function nearestRestaurantName(hotspot, restaurants) {
  const closest = [...restaurants].sort((a,b) => haversineKm(a, hotspot) - haversineKm(b, hotspot))[0];
  return closest ? t('results.nearby', { name: closest.name }) : t('results.midpointNearby');
}

function externalMapUrl(point) {
  if (isKakaoMapsConfigured) return `https://map.kakao.com/link/map/${encodeURIComponent(point.name || 'MeetHalfway')},${point.lat},${point.lng}`;
...
function resultsHtml(snapshot, restaurants, hotspots) {
  ...
  const candidateCards = hotspots.length ? hotspots.map((hotspot, index) => `
    <article class="candidate-card"><span class="candidate-rank">${escapeHtml(t('results.candidate', { n: index + 1 }))}</span><h3>${escapeHtml(nearestRestaurantName(hotspot, restaurants))}</h3><p>${escapeHtml(t('results.restaurantCell', { count: hotspot.restaurantCount }))} · ${escapeHtml(t('results.fromCenter', { distance: hotspot.centerDistanceKm.toFixed(1) }))}</p><a href="${externalMapUrl(hotspot)}" target="_blank" rel="noreferrer">${escapeHtml(t('results.openMap'))}</a></article>`).join('') : `<div class="empty-state"><strong>${escapeHtml(t('results.fallbackTitle'))}</strong><span>${escapeHtml(t('results.fallbackBody'))}</span></div>`;
  const restaurantCards = [...restaurants].sort((a,b) => haversineKm(a, center) - haversineKm(b, center)).slice(0,12).map((restaurant) => `
    <a class="restaurant-row" href="${externalMapUrl(restaurant)}" target="_blank" rel="noreferrer"><span class="restaurant-dot" aria-hidden="true"></span><span><strong>${escapeHtml(restaurant.name)}</strong><small>${escapeHtml(restaurant.cuisine || t('results.restaurant'))} · ${escapeHtml(t('results.fromCenter', { distance: haversineKm(restaurant, center).toFixed(1) }))}</small></span><span aria-hidden="true">↗</span></a>`).join('');
  return `
    <div class="result-summary"><div><span>${escapeHtml(t('map.center'))}</span><strong>${center.lat.toFixed(5)}, ${center.lng.toFixed(5)}</strong></div><div><span>${escapeHtml(t('results.avgDistance'))}</span><strong>${avg.toFixed(1)}km</strong></div><div><span>${escapeHtml(t('results.maxDistance'))}</span><strong>${max.toFixed(1)}km</strong></div><div><span>${escapeHtml(t('results.restaurantCount'))}</span><strong>${restaurants.length}</strong></div></div>
    <div class="result-note">${escapeHtml(t('results.note'))}</div>
    <div class="candidate-grid">${candidateCards}</div>
    ${restaurantCards ? `<div class="restaurant-list"><div class="subheading"><h3>${escapeHtml(t('results.restaurantHeading'))}</h3><span>${escapeHtml(t('results.dataSource'))}</span></div>${restaurantCards}</div>` : ''}`;
}
...
async function renderMeeting(shareId) {
  ...
  try {
    ...
    if (closed) {
      ...
      try {
        const restaurants = await findRestaurants(calculatedCenter);
        if (version !== routeVersion) return;
        const hotspots = buildHotspots(restaurants, snapshot.participants, calculatedCenter);
        resultPanel.innerHTML = resultsHtml(snapshot, restaurants, hotspots);
        activeMap?.render(snapshot.participants, calculatedCenter, restaurants.slice(0, APP_CONFIG.maxRestaurantMarkers), hotspots);
      } catch (error) {
        if (version !== routeVersion) return;
