...
import { mountRecommendationScene } from './lib/threeFx.js';
...
function resultsHtml(snapshot, allPlaces, hotspots, selectedIndex = 0, selectedCategory = 'all') {
  ...
  return `
    ...
    ${hotspots.length ? `<section class="three-recommendation-shell" aria-label="${escapeHtml(t('results.spatialTitle'))}"><div id="recommendation-three" class="three-recommendation"><div class="three-recommendation-placeholder"><span></span><strong>${escapeHtml(t('results.spatialTitle'))}</strong><small>${escapeHtml(t('results.spatialHint'))}</small></div></div><div class="three-recommendation-legend"><span><i class="three-legend-core"></i>${escapeHtml(t('results.midpointCore'))}</span><span><i class="three-legend-point"></i>${escapeHtml(t('results.spatialCandidate'))}</span><em>${escapeHtml(t('results.spatialToggle'))}</em></div></section>` : ''}
...
function activateResultExperience(resultPanel, snapshot, allPlaces, hotspots, center) {
  ...
  const paint = () => {
    ...
    const threeContainer = resultPanel.querySelector('#recommendation-three');
    if (threeContainer && hotspots.length) {
      activeThreeScene = mountRecommendationScene(threeContainer, {
        center,
        hotspots,
        selectedIndex,
        onSelect: toggleCandidate,
        labels: {
          title: t('results.spatialTitle'),
          hint: t('results.spatialHint'),
          pick: t('results.spatialPick')
        }
      });
    }
