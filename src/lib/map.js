...
function candidateLabel(hotspot, index) {
  const name = hotspot.areaLabel || hotspot.areaName || t('results.candidate', { n: index + 1 });
...
function emitCandidateSelect(container, index) {
  container?.dispatchEvent(new CustomEvent('meet-halfway:candidate-select', {
    detail: { index },
    bubbles: false
  }));
}
...
class LeafletMeetingMap {
  ...
  render(participants, center, places = [], hotspots = [], options = {}) {
    ...
    if (center) {
      ...
      const recommendationRadiusKm = Number(hotspots[0]?.recommendationRadiusKm || 0);
      if (recommendationRadiusKm > 0) {
        const zone = L.circle([center.lat, center.lng], {
          radius: recommendationRadiusKm * 1000,
          weight: 2,
          dashArray: '7 7',
          color: '#198e73',
          fillColor: '#33b892',
        ...
        zone.bindTooltip(t('map.recommendationZone', { distance: recommendationRadiusKm.toFixed(1) }), { direction: 'top' });
        this.layers.push(zone);
      }
    }
    ...
    hotspots.forEach((hotspot, index) => {
      ...
      pin.bindTooltip(`${escape(t('results.candidate', { n: index + 1 }))} · ${escape(hotspot.areaLabel || hotspot.areaName || '')}`, { permanent: selected, direction: 'top', offset: [0, -36], className: selected ? 'map-label map-label--selected' : 'map-label' });
      pin.on('click', () => emitCandidateSelect(this.container, index));
...
class GoogleMeetingMap {
  ...
  render(participants, center, places = [], hotspots = [], options = {}) {
    ...
    if (center) {
      ...
      const recommendationRadiusKm = Number(hotspots[0]?.recommendationRadiusKm || 0);
      if (recommendationRadiusKm > 0) {
        this.addCircle(position, { radius: recommendationRadiusKm * 1000, strokeColor: '#198e73', strokeOpacity: 0.58, strokeWeight: 2, fillColor: '#33b892', fillOpacity: 0.025 }, `<strong>${escape(t('map.recommendationZone', { distance: recommendationRadiusKm.toFixed(1) }))}</strong>`);
      }
    }
    hotspots.forEach((hotspot, index) => {
      ...
      const candidateCircle = this.addCircle(position, { radius: selected ? 240 : 180, strokeColor: selected ? '#b33a1f' : '#c58a13', strokeOpacity: 1, strokeWeight: selected ? 4 : 3, fillColor: selected ? '#ff795c' : '#ffc857', fillOpacity: 0.62 }, candidateLabel(hotspot, index));
      candidateCircle.addListener('click', () => emitCandidateSelect(this.container, index));
      overviewBounds.extend(position);
...
class KakaoMeetingMap {
  ...
  render(participants, center, places = [], hotspots = [], options = {}) {
    ...
    if (center) {
      ...
      const recommendationRadiusKm = Number(hotspots[0]?.recommendationRadiusKm || 0);
      if (recommendationRadiusKm > 0) {
        this.addCircle(center, { radius: recommendationRadiusKm * 1000, strokeColor: '#198e73', strokeWeight: 2, strokeOpacity: 0.58, fillColor: '#33b892', fillOpacity: 0.025 }, `<strong>${escape(t('map.recommendationZone', { distance: recommendationRadiusKm.toFixed(1) }))}</strong>`);
      }
    }
    hotspots.forEach((hotspot, index) => {
      const selected = Boolean(focusCandidate && hotspot.id === focusCandidate.id);
      ...
      window.kakao.maps.event.addListener(item.circle, 'click', () => emitCandidateSelect(this.container, index));
      overviewBounds.extend(item.location);
