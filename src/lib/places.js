...
async function osmDiscoverPlaces(center, participants) {
  const radiusKm = adaptiveSearchRadiusKm(center, participants);
  const radius = Math.round(Math.min(38000, Math.max(APP_CONFIG.overpassRadiusMeters, radiusKm * 1000)));
  const query = `[out:json][timeout:28];nwr["amenity"~"^(restaurant|cafe|fast_food|food_court|bar|pub)$"](around:${radius},${center.lat},${center.lng});out body center qt 1400;`;
  let lastError = null;
