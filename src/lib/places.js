...
export async function findRestaurants(center) {
  if (isKakaoMapsConfigured) {
    try {
      return await kakaoFindRestaurants(center);
    } catch (error) {
      console.warn('[MeetHalfway] Kakao restaurant search unavailable; falling back to OpenStreetMap/Overpass.', error);
      return osmFindRestaurants(center);
    }
  }
  if (isGoogleMapsConfigured) {
    try {
      return await googleFindRestaurants(center);
    } catch (error) {
      console.warn('[MeetHalfway] Google restaurant search unavailable; falling back to OpenStreetMap/Overpass.', error);
      return osmFindRestaurants(center);
    }
  }
  return osmFindRestaurants(center);
