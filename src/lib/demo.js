...
// Compact demo clusters around the geometric midpoint. These intentionally
// mimic realistic downtown meeting choices instead of spreading suggestions
// across an entire city.
const demoClusters = [
  { lat: 37.5547, lng: 126.9707, label: '서울역', offset: 0, count: 12 },
  { lat: 37.5608, lng: 126.9860, label: '명동', offset: 20, count: 11 },
  { lat: 37.5657, lng: 126.9769, label: '시청', offset: 40, count: 10 }
];

export const DEMO_PLACES = demoClusters.flatMap((cluster) => Array.from({ length: cluster.count }, (_, index) => {
  const angle = (index / cluster.count) * Math.PI * 2;
  const ring = 0.0011 + (index % 3) * 0.00042;
  ...
  return {
    ...
    address: `${cluster.label} 추천권 · 데모 예시 데이터`,
